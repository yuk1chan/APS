...
void walk(std::vector<Particle> &walker, int &N, int const &N0, double &Eref,
  ...
  for (int i = Nin - 1; i > 0; i--) {
    ...
    if (w > 1) {                     // 粒子をw-1個増やす
      ...
      Esum += walker[i].w * w * Enew; // 同じ位置にw個の粒子が存在するため。
    } else if (w == 1) {              // 粒子数に変化なし
      Esum += walker[i].w * Enew;
    } else {                     // 粒子が消滅
...
/*
  ...
  @param[in] size
*/
void data(std::vector<Particle> &walker, int const &N, double &Esum,
          double const &Eave, int const &imcs, double const &size) {
...
int main(int argc, char const *argv[]) {
  ...
  for (imcs = 1; imcs <= mcs; imcs++) {
    walk(walker, N, N0, Eref, Eave, ds, dt, a, lambda);
    data(walker, N, Esum, Eave, imcs, size);
  }
