...
void initial(double *x, double *y,
             int &N, int &N0, double &Vref,
             int &size, double &ds, double &dt, int &mcs,
             double &Esum, double &nequil, double &a, double &v);
...
void walk(double *x, double *y,
          int &N, int &N0, double &Vref,
          double &Vave, double const ds, double const dt, double const &a, double const &v);
...
void data(double *x, double y,
          double *psi, int N, double Vref, double Vave,
...
double r(double const &x, double const &y){
  return sqrt(x*x + y*y);
}
...
void initial(double *x, double *y,
             ...
             double &Esum, int &nequil, double &a, double &v){
...
void walk(double *x, double *y,
  ...
  /* 全ての粒子について*/
  for( i = Nin; i >= 0; i-- ){ // Ninから始まるのは粒子の消滅を考える必要があるため
